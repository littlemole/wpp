                                ...
                                struct curl_sockaddr *address)
{
  ...
  if(purpose == CURLSOCKTYPE_IPCXN && address->family == AF_INET) 
  {
    ...
    else 
	{
      ...
      socket_map.insert(std::pair<reprocurl::impl::socket_t,boost::asio::ip::tcp::socket *>(sockfd, tcp_socket));
    }
  }
...
void CurlMulti::remove(reprocurl::impl::CURL* c)
{
	curl_multi_remove_handle(multi_,c);
}

bool CurlMulti::add(reprocurl::impl::CURL* c)
{
...
bool CurlMulti::onSocket(reprocurl::impl::socket_t fd, short kind, CurlEasy* easy)
{
	int action =
...
int CurlMulti::on_sock_cb(CURL *curl, reprocurl::impl::socket_t sock, int what, CurlEasy* easy)
{
	easy->what_ = what;
...
repro::Future<CurlEasy::Ptr> CurlEasy::perform()
{
...
CurlEasy::Ptr CurlEasy::header(const std::string& key, const std::string& val)
{
    std::ostringstream oss;
    oss << key << ":" << val;
    headers_ = (reprocurl::impl::curl_slist*)curl_slist_append((::curl_slist*)headers_, oss.str().c_str() );
	return shared_from_this();
}
...
size_t CurlEasy::on_rcvHeaders_cb(void *buffer, size_t size, size_t nmemb)
{
    ...
    if ( len > 0)
    {
		...
		{
			std::string key = prio::trim(s.substr(0,pos));
			std::string val = prio::trim(s.substr(pos+1,s.size()-pos-3));
